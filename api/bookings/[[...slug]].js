...
module.exports = async (req, res) => {
  ...
  const id = Array.isArray(slug) && slug.length ? slug[0] : null;

  // ===== /api/bookings/:id — عام، يستخدمه المراجع لعرض تفاصيل موعده =====
  if (id) {
  ...
  // ===== POST /api/bookings — عام، إنشاء حجز جديد =====
  if (req.method === 'POST') {
  ...
  // ===== GET /api/bookings — بصلاحية دخول، لوحة الأدمن =====
  if (req.method === 'GET') {
